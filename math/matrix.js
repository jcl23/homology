...
export const swapRows = (matrix, row1, row2) => {
    const newMatrix = matrix.map(row => [...row]);
    [newMatrix[row1], newMatrix[row2]] = [newMatrix[row2], newMatrix[row1]];
    return newMatrix;
};
export const swapCols = (matrix, col1, col2) => {
    const newMatrix = matrix.map(row => [...row]);
...
export const multiplyMatrix = (matrix, scalar) => {
    return matrix.map(row => row.map(val => val * scalar));
};
export const multiplyRow = (matrix, row, scalar) => {
    return matrix.map((r, i) => i === row ? r.map(val => val * scalar) : r);
};
export const multiplyCol = (matrix, col, scalar) => {
...
function matrixAddRow(A, row1, row2, factor) {
...
function matrixSwapRows(A, row1, row2) {
    const newMatrix = A.map(row => [...row]);
...
function matrixSwapCols(A, col1, col2) {
    const newMatrix = A.map(row => [...row]);
...
function columnEmpty(A, col) {
    return A.every(row => row[col] === 0);
}
export function isZeroMatrix(A) {
    return A.every(row => row.every(value => value === 0));
}
export function rankImage(A) {
    let rank = 0;
...
export function computeTorsion(A) {
...
//     return matrix;
// }
export const reduce = function (B) {
